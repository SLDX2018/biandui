...
{
    // 两个灯条体型相似
    if (Led_stick[0].rect.size.height * 0.6f < Led_stick[1].rect.size.height
        && Led_stick[0].rect.size.height * 1.4f > Led_stick[1].rect.size.height)
    {
        ...
        if (armor_width > Led_stick[0].rect.size.width
            ...
            && armor_width > (Led_stick[0].rect.size.width + Led_stick[1].rect.size.width) *2)//原本*3,防止拖影,导致装甲板灯条宽度增加,使之乘2
        {
            ...
            {
                // 长宽比判断
                if (h_max * 4.0f > rect.width && h_max < 1.4f * rect.width);//4.0,1.2
                {
                    return true;
                }
//                if (h_max * 7.0f > rect.width && h_max < 1.6f * rect.width);//4.0,1.2
//                {
//                    return true;
//                }
...
int armor::get_average_intensity(const Mat& img) {
    ...
    average_intensity = static_cast<int>(mean(roi).val[0]);//第一通道均值(为灰度图)
...
bool ArmorDetector::DetectArmor(Mat& img, Rect roi_rect)
{
    ...
    vector<cv::Mat> bgr;
    split(roi_image, bgr); //Channel separation
    Mat result_img;
    if (color_ == 0)//blue team
    {
        subtract(bgr[2], bgr[0], result_img);//red
    }
    else
    {
        subtract(bgr[0], bgr[2], result_img);//blue
    }
    ...
    imshow("灰度", binary_brightness_img);
    imshow("通道相减", binary_color_img);
#endif
    ...
    for (size_t i = 0; i < contours_brightness.size(); i++)
    {
        ...
        {
            //检测点是否在轮廓内的函数
            if (pointPolygonTest(contours_light[ii], contours_brightness[i][0], false) >= 0.0)//颜色轮廓包围灰度轮廓思想
            {
                double length = arcLength(contours_brightness[i], true); // 灯条周长
                if (length > 15 && length < 4000)//4000
                {                    // 使用拟合椭圆的方法要比拟合最小矩形提取出来的角度更精确
                    if (contours_brightness.at(i).size() <= 5 ) continue;//防止椭圆拟合报错//
...
#endif
                    if (fabs(RRect.angle) <= 40)  // 超过一定角度的灯条不要
                    {
                        LED_Stick r(RRect);
    ...
    for (size_t i = 0; i < LED_Stick_v.size(); i++)
    {
        ...
        {
            armor arm_tmp(LED_Stick_v[i], LED_Stick_v[j]);
            if (arm_tmp.error_angle < 7.0f)
            {
...
//					// TODO(cz): 推荐使用255值的面积进行判断
    ...
    if(found_flag)
    {
        ...
        if(points_2d_.at(0)==Point2f(0,0)&&points_2d_.at(1)==Point2f(0,0)&&points_2d_.at(2)==Point2f(0,0)&&points_2d_.at(3)==Point2f(0,0))
            return 0;
        // 计算当前装甲板类型，到后面task中还有滤波，可以有误差
        float armor_h = target.rect.height;
        float armor_w = target.rect.width;
        if(armor_w / armor_h < 2.8f)
        {is_small_ = 1;
        cout<<"is_small"<<endl;}
        else
        {is_small_ = 0;
...
int ArmorDetector::ArmorDetectTask(Mat& img, OtherParam other_param)
{
    ...
    if (DetectArmor(img, roi))
    {
        ...
        DrawTarget(img);
#endif
        bool final_armor_type = getTypeResult(is_small_);//0-big armor    1-small armor

        solve_angle_.Generate3DPoints((uint)final_armor_type, Point2f());
        solve_angle_.getAngle(points_2d_, other_param.bullet_speed,angle_x_,angle_y_,distance_,img);   // pnp姿态结算
...
bool ArmorDetector::getTypeResult(bool is_small)
{
    ...
    int vote_cnt[2] = {0};//vote_cnt[0]--big armor   vote_cnt[1]--small armor
