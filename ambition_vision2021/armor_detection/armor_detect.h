 ...
 * @brief debug
 */
#define Draw_Target //两条交叉线
#ifdef DEBUG_ARMOR_DETECT
// ------ debug armor ------
//#define SHOW_BINARY_IMAGE //用于显示两幅二值化图
//#define SHOW_LIGHT_PUT_TEXT //灯条拟合矩形角度
  

#define SHOW_LIGHT_CONTOURS //展示轮廓拟合矩形
//#define SHOW_ARMOR_PUT_TEXT //展示装甲板宽高比信息
#define SHOW_DRAW_RECT //装甲板内轮廓

#define SHOW_DRAW_SPOT //装甲板中心点

//#define SHOW_ROI_RECTANGLE //roi区
...
public:
    ...
    float match_factor; // 匹配强度， 用于灯条匹配.角度，越小越好
};
/**
 * @brief 装甲板相关数据信息
 */
...
public:
    ...
    void draw_spot(Mat& img, Point2f roi_offset_point) const;   //画出圆心
...
public:
    ArmorDetector() {
//        solve_angle_ = SolveAngle(SHOR_X, SHOR_Y, SHOR_Z, PTZ_TO_BARREL); // 短焦角度解算
    ...
    void getAngle(float& yaw, float& pitch,float& distance)
    {
        yaw = (angle_x_);
        pitch = (angle_y_);
        distance = static_cast<float>(distance_);
    }
...
//    int short_offset_y_ = 135;
    int short_offset_x_ = 100;
    int short_offset_y_ = 100;
    int long_offset_x_ = 100;
    int long_offset_y_ = 100;
    int color_th_ = 16;    //ICRA-16
    int gray_th_ = 50;     //ICRA-60


private:
    // 相关类声明
    SolveAngle solve_angle_= SolveAngle(1,1,1,0);
...
private:
    ...
    float angle_x_ = 0;
    float angle_y_ = 0;
    vector<Point2f> points_2d_; //补偿，用于调整准心
